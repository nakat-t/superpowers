...
| Cline | `.cline/plugins/superpowers.ts` (declared via root `package.json` `cline.plugins`) | in-process: bundled `skills/` auto-discovered; `registerMessageBuilder` injects user message (per-call dedup guard; compaction-aware) | `clineToolMapping()` inline **and** `references/cline-tools.md` | `tests/cline/` | `cline plugin install` git URL (CLI/Kanban/SDK) |
